...
class Command(BaseCommand):
    help = 'Load ALL HS codes from info.txt file, handling variations in data format'
            ...
            try:
                if len(parts) >= 10:
                    ...
                    if len(parts) >= 12:
                        ...
                        # Shorter format (9 parts) - excise rate missing
                        duty_rate = float(parts[6].strip()) if parts[6].strip() else 0.0
                        vat_rate = float(parts[7].strip()) if parts[7].strip() else 0.0
                        excise_rate = 0.0  # Default to 0
                        required_certs = parts[8].strip() if len(parts) > 8 else '[]'
                    else:
                        self.stdout.write(f'  WARNING: Line {i+1} has only {len(parts)} parts, skipping')
                        error_count += 1
                        continue
